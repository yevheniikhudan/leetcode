...
public class PacificAtlanticWaterFlow {
    public List<List<Integer>> pacificAtlantic(int[][] heights) {
        ...
        // Pacific
        for (int c = 0; c < COLS; c++) q.offer(new int[]{0, c});
        for (int r = 0; r < ROWS; r++) q.offer(new int[]{r, 0});
        bfs(heights, q, pac, dirs);
        // Atlantic
        q.clear();
        for (int c = 0; c < COLS; c++) q.offer(new int[]{ROWS-1, c});
        for (int r = 0; r < ROWS; r++) q.offer(new int[]{r, COLS-1});
